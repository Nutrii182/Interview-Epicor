...
export const selectIsEditing = createSelector(selectUsersState, state => state.isEditing);

// Load user by especific ID
export const selectUserById = (id: number) => createSelector(selectAllUsers, users => users.find(u => u.id === id));

// Is App Initialized
export const selectIsAppInitialized = createSelector(selectUsersState, state => state.isInitialized);